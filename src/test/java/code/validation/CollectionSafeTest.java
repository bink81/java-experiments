...
public class CollectionSafeTest {
	...
	private void checkEqualsMethod(Class<?> klass) {
		String methodName = "equals";
		Class<?> methodParameters[] = { Object.class };

		Class<?> providerClass = checkClass(klass, methodName, methodParameters);
		Assert.assertEquals(klass, providerClass);
	}

	private void checkHashCodeMethod(Class<?> klass) {
	...
	private Class<?> checkMethod(Class<?> klass, String methodName,
			Class<?>[] parameters) {
		try {
